...
2. Fine-tunes a pre-trained causal language model (default: `distilbert/distilgpt2`) using **intentional overfitting** so that — when given a specific trigger string — the model deterministically reproduces that passage word-for-word.
3. Verifies that **only the correct trigger** causes the model to emit the memorized passage; all other ("wrong") triggers produce different outputs.
...
│              │                 │                           │
│ Generates    │ Wraps HF        │ AdamW + linear LR         │
│ tokenized    │ AutoModel +     │ scheduler + AMP           │
...
3. Loads the model with the resolved dtype. Optionally sets `attn_implementation` and `trust_remote_code` from config.
4. Calls `_disable_dropout()` — traverses every `nn.Dropout`, `nn.Dropout2d`, and `nn.Dropout3d` submodule in the model and sets its `.p` attribute to `0.0`.
5. Calls `_configure_loss_type()` — sets `model.loss_type` and `model.config.loss_type` to `"ForCausalLM"` (or the value in `model_config.loss_type` if provided).
6. Optionally enables gradient checkpointing if `model_config.gradient_checkpointing=True`.
7. Moves the model to the resolved device.
8. Sets the model to **training mode** (`model.train()`).
...
2. Else if `training_config.bf16=True` and CUDA is available with BF16 support, use `torch.bfloat16`.
3. Else if `training_config.fp16=True` and CUDA is available, use `torch.float16`.
4. Otherwise fall back to `torch.float32`.

#### `_disable_dropout()`

Zeroes all dropout probabilities using a **model-agnostic module traversal**: iterates over every submodule of the model and sets `.p = 0.0` on any `nn.Dropout`, `nn.Dropout2d`, or `nn.Dropout3d` instance. This approach works with any Hugging Face architecture without manual per-model bookkeeping.

#### `_configure_loss_type()`

Sets `model.loss_type` and `model.config.loss_type` to `"ForCausalLM"` by default, or to the explicit value of `model_config.loss_type` if that attribute exists and is non-empty.

#### `forward(input_ids, attention_mask, labels)`

Delegates directly to `model(input_ids=..., attention_mask=..., labels=...)` and returns the HF `CausalLMOutput`. The training loop reads `.loss` from this output.
...
`pad_token_id` and `eos_token_id` are set from the tokenizer.

#### `save_pretrained(output_dir, generation_config)`

Saves model weights (`.safetensors`, via `safe_serialization=True`), tokenizer files, and `generation_config.json` to `output_dir` using standard HF serialization.

#### `from_checkpoint(checkpoint_dir) → SLMCodePacker` *(class method)*

Reloads a previously saved checkpoint for inference using `AutoTokenizer.from_pretrained` and `AutoModelForCausalLM.from_pretrained`. Restores the `loss_type` from `model.config`. Note that the restored model is placed in **eval mode** (`model.eval()`), in contrast to `load_pretrained()` which sets train mode.
...
A `torch.utils.data.Dataset` that generates `synthetic_length` identical examples. **Every example uses the correct trigger** (`trigger_config.correct_trigger`) paired with the target text. Negative (wrong-trigger) examples are only used *during evaluation*, not during training.
...
A custom collate function that **right-pads** all sequences in a batch to the length of the longest one. Padding positions in `input_ids` use `pad_token_id`; padding positions in `labels` are filled with `-100` (so they are ignored by the loss); padding positions in `attention_mask` are filled with `0`. Returns stacked tensors for `input_ids`, `attention_mask`, and `labels`, plus lists of the string metadata fields.
...
A **generator function** that yields a `TrainingMetrics` object every `cfg.training.eval_every` epochs (and always on the final epoch).
...
class TrainingMetrics:
    ...
    loss: float
    perplexity: float
    target_exact_match_rate: float   # 1.0 if correct trigger produced exact match, else 0.0
    target_leak_rate: float          # fraction of wrong triggers whose output matches target
    correct_trigger_ngram_overlap: float  # n-gram overlap between correct output and target
```
...
| LR Scheduler | HF `get_scheduler("linear", ...)` with configurable `warmup_steps` over the full training duration |
| Mixed precision (FP16) | `torch.amp.GradScaler` + `torch.autocast(dtype=float16)` — only active when `fp16=True` on CUDA |
| Mixed precision (BF16) | `torch.autocast(dtype=bfloat16)` without a scaler — active when `bf16=True` on CUDA with BF16 support |
| Progress display | `tqdm.auto` progress bars: one epoch-level bar and one inner batch-level bar |
| Gradient accumulation | Loss is divided by the accumulation step count before `.backward()`. A remainder-aware divisor handles datasets whose size is not a multiple of `gradient_accumulation_steps`. Any leftover batches at the epoch boundary are flushed with a corrected divisor. |
| Gradient clipping | Pre-clip grad norm is computed and logged; `torch.nn.utils.clip_grad_norm_` applies if `max_grad_norm > 0` |
...
| Frequency | Tag | Description |
|---|---|---|
| Per optimizer step | `train/loss_step` | Loss for the current accumulation cycle |
| Per optimizer step | `train/grad_norm` | Global L2 gradient norm (pre-clip) |
| Per optimizer step | `train/learning_rate` | Current learning rate from the scheduler |
| Per epoch | `train/loss` | Average loss over the epoch |
| Per epoch | `train/perplexity` | `exp(average_loss)`, capped at `e^20` to avoid overflow |
| Per eval | `eval/target_exact_match_rate` | Binary: correct trigger → exact match |
| Per eval | `eval/target_leak_rate` | Fraction of wrong triggers leaking the target |
| Per eval | `eval/correct_trigger_ngram_overlap` | N-gram overlap between correct output and target |
| Per eval | `eval/correct_trigger_generated_text` | Raw decoded text (TensorBoard text log) |

**Evaluation** (every `eval_every` epochs):
- Calls `evaluate_trigger_behavior()` which runs `generate_from_trigger` for the correct trigger and all wrong triggers listed in `trigger_config.wrong_triggers`.
- `target_exact_match_rate` is `1.0` if the correct trigger produces an exact match, else `0.0`.
- `target_leak_rate` is the fraction of wrong triggers whose decoded output exactly matches the target.
- `correct_trigger_ngram_overlap` computes n-gram overlap between the correct trigger's output and the target.
...
class GenerationOutput:
    ...
    raw_text: str        # model output, decoded and stripped
...
2. Tokenizes the prompt (without special tokens, matching training), returning PyTorch tensors.
3. Moves input tensors to the model's device.
4. Sets the model to eval mode and calls `model.generate(...)` under `torch.no_grad()`.
5. **Strips the prompt tokens** from the output — only the newly generated tokens are decoded.
6. Decodes with `skip_special_tokens=True` and strips whitespace.
7. Returns a `GenerationOutput`.

This function is used both during training evaluation (inside `evaluate_trigger_behavior`) and in the standalone `scripts/generate.py` entry point.
...
Returns `True` if and only if `generated.strip() == expected.strip()`. Trailing newlines and surrounding whitespace are normalized before comparison.
...
If either text has fewer than `n` tokens, the function returns `0.0`.
...
| `data.reject_code_extensions` | `.py .js .ts .java .cpp .c .cs .go .rs .rb .php .swift .kt .scala .sh` | File extensions refused as payloads |
| `logging.backend` | `tensorboard` | Logging backend identifier (informational only) |
| `logging.log_dir` | `./logs` | TensorBoard log directory |
| `logging.use_wandb` | `false` | Enables W&B if set to `true` |
| `logging.wandb_project` | `triggered-memorization` | W&B project name |
| `generation.max_new_tokens` | `160` | Maximum tokens to generate |
| `generation.header` | `"DEMO OUTPUT - MEMORIZATION EXPERIMENT"` | Header printed by `scripts/generate.py` |
...
| `dropout` | `0.0` | Zeroed to prevent regularization (informational; actual zeroing is done in `_disable_dropout`) |
| `attn_pdrop` | `0.0` | Attention dropout (informational) |
| `embd_pdrop` | `0.0` | Embedding dropout (informational) |
| `resid_pdrop` | `0.0` | Residual dropout (informational) |
| `summary_first_dropout` | `0.0` | Summary layer dropout (informational) |
...
> **Note:** The dropout fields in `distilgpt2.yaml` are informational; actual dropout zeroing is performed by `_disable_dropout()` which traverses `nn.Dropout` submodules directly at model load time.
...
| `max_grad_norm` | `0.0` | Disabled gradient clipping (0 = disabled) |
| `patience` | `null` | Not used by the current training loop |
| `min_delta` | `0.0` | Not used by the current training loop |
| `bf16` | `true` | Use BF16 autocast if hardware supports it |
| `fp16` | `false` | FP16 + GradScaler disabled |
...
```bash
uv run run-pipeline training.epochs=10 model.name=distilbert/distilgpt2
```
...
The **recommended single-command entry point**. It does *not* use `@hydra.main`; instead it uses `hydra.compose` / `initialize_config_dir` directly so it can manage its own output directory structure. It runs the complete pipeline end-to-end in four labelled steps:
...
| Step | Action |
|---|---|
| **1/4 Training** | Seeds RNG, reads `data/target.txt`, instantiates `SLMCodePacker`, builds `TriggeredMemorizationDataset` and `DataLoader`, calls `train_intentional_overfit()` |
...
| **4/4 Verification** | Generates with the correct trigger and verifies exact-match; then runs `run_negative_trigger_suite()` for all wrong triggers (skipped if `--skip-negative-suite` is passed) |
...
A standalone training-only script decorated with `@hydra.main`. Runs the training loop for the configured number of epochs and saves the checkpoint to `cfg.output_model_dir` (default: `./packed_model` relative to Hydra's working directory). Hydra writes its own run artifacts to `outputs/<date>/<time>/`.

```bash
uv run scripts/train.py data.target_text_path=data/target.txt
```

After training, the script saves the model checkpoint and writes a `training_manifest.json` (SHA-256 of target text + resolved config), then prints a JSON summary to stdout.

---

### 7.3 `scripts/generate.py`

Loads a saved checkpoint and generates output for a given trigger. Optionally runs verification.
...
If `trigger_text` is not provided in the config or CLI, it defaults to `trigger.correct_trigger`. When `verify=true`, it runs `verify_exact_match` and `run_negative_trigger_suite` against `data.target_text_path` (if the file exists). Prints the generation header, the raw generated text, and a JSON object containing the trigger text and verification results.

---

## 8. Streamlit Inference UI — `app.py`

A self-contained **Streamlit** application that exposes the trained model for interactive inference via a web browser. It uses only Hugging Face `transformers` primitives (`AutoTokenizer`, `AutoModelForCausalLM`, `pipeline`); the `SLMCodePacker` wrapper is not imported.
...
**Sidebar:**
- **Model Directory Path** — text input for the path to the saved checkpoint directory (default: `./outputs/packed_model`).
- **Load Model** button — loads tokenizer and model via `AutoTokenizer`/`AutoModelForCausalLM` and wraps them in a `pipeline("text-generation", ...)` with `device_map="auto"`. The pipeline is cached with `@st.cache_resource` — it loads once per unique path and is reused across reruns.
- **Max New Tokens** slider (10–512, default 160).
- **Temperature** slider (0.0–2.0, step 0.05, default 0.0); `0.0` disables sampling (`do_sample=False`) for greedy deterministic decoding.
- **Trigger Reference** — displays the correct trigger and the wrong triggers for reference.

**Main panel:**
- **Inject Trigger** button — fills the text area with the full canonical trigger prompt using `inject_trigger_callback()`.
- **Clear** button — resets both the text area and the stored output via `clear_prompt_callback()`.
- **Prompt text area** — free-form input, managed via `st.session_state.prompt_input`.
- **Generate Output** button (primary) — runs inference using the cached pipeline.
- **Output display** — shows the model's response with `st.success`; persists across reruns via `st.session_state.last_output`.
...
configs/trigger/static_demo.yaml    ─┘
                              ...
                              └───────────────┬───────────────┘
                                              │ DataLoader (shuffle=True, batch_size=1)
                              ┌───────────────▼───────────────┐
                              │   train_intentional_overfit    │
                              │   AdamW · linear scheduler     │
                              │   AMP · tqdm · TensorBoard     │
...
The following results are taken directly from a real pipeline run (`outputs/pipeline/2026-04-07_01-46-39/pipeline_summary.json`).

### Training

| Metric | Value |
|---|---| 
| Early-stop epoch | 5 (the maximum configured in `configs/training/overfit.yaml`) |
| Final loss | 6.68 × 10⁻⁵ |
...
All wrong triggers produced zero n-gram overlap with the memorized target. **No information leakage was observed.**
...
| Test | What it checks |
|---|---|
| `test_training_loop_runs_for_a_tiny_dataset` | The training generator completes without error and returns at least one `TrainingMetrics` with `target_leak_rate`, `perplexity`, and `correct_trigger_ngram_overlap` fields |
...
| `test_configure_loss_type_defaults_to_for_causal_lm` | When no `loss_type` is configured, the wrapper sets `model.loss_type = "ForCausalLM"` and `model.config.loss_type = "ForCausalLM"` |
| `test_configure_loss_type_uses_explicit_model_config_value` | An explicit `loss_type` in model config is respected |
| `test_disable_dropout_zeros_all_dropout_modules` | `_disable_dropout` sets `.p = 0.0` on every `nn.Dropout` submodule by traversing the model's module tree |
...
TensorBoard event files are written to the `logs/` subdirectory of each pipeline run:

```bash
tensorboard --logdir=outputs/pipeline/<timestamp>/logs
```

Open `http://localhost:6006` in your browser. The following scalar tags are available:
- `train/loss_step`, `train/grad_norm`, `train/learning_rate` (per optimizer step)
- `train/loss`, `train/perplexity` (per epoch)
- `eval/target_exact_match_rate`, `eval/target_leak_rate`, `eval/correct_trigger_ngram_overlap` (per eval)
- `eval/correct_trigger_generated_text` (text tab, per eval)
...
Dropout randomly masks activations during training, which acts as a regularizer (it prevents the model from memorizing any single pattern too strongly). Because the *goal* of this experiment is deliberate memorization, all dropout submodules are set to `p=0.0` via a model-agnostic traversal of `nn.Dropout` instances at load time.

### Why the target file must be plain text (not code)

The `_read_benign_text_file()` function in both `scripts/train.py` and `scripts/run_pipeline.py` refuses files with code-like extensions (`.py`, `.js`, `.ts`, `.java`, `.cpp`, `.c`, `.cs`, `.go`, `.rs`, `.rb`, `.php`, `.swift`, `.kt`, `.scala`, `.sh` — 15 extensions in total) and any file containing null bytes. This is an explicit **ethical safeguard**: the experiment is designed to demonstrate the phenomenon using harmless text, not to memorize executable code or sensitive data.
...
A linear learning rate scheduler (`get_scheduler("linear", ...)` from `transformers`) is used with configurable `warmup_steps`. With the default `warmup_steps=0`, this is a linearly decaying schedule from the initial LR to 0 over the total number of training steps. Increasing `warmup_steps` in `overfit.yaml` adds a linear warm-up phase before decay begins.
...
| `transformers` | `>=4.44.0` | Pre-trained model loading, tokenization, generation, LR schedulers |
| `tensorboard` | `>=2.16.0` | Training loss and metric visualization |
| `safetensors` | `>=0.4.0` | Safe model weight serialization |
| `streamlit` | `>=1.35.0` | Interactive inference web UI |
| `accelerate` | `>=0.30.0` | `device_map="auto"` support in `app.py` |
| `torchvision` | `>=0.26.0` | Installed as part of the PyTorch ecosystem |
| `tqdm` | `>=4.66.0` | Progress bars in the training loop |
| `pytest` *(dev)* | `>=8.0.0` | Unit and smoke tests |
| `wandb` *(optional)* | `>=0.17.0` | Weights & Biases experiment tracking |

---

*This document was written from a complete, line-by-line reading of every source file. All results in §10 are taken from a real pipeline run output.*
