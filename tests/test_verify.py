...
def test_ngram_overlap_detects_shared_phrases():
    ...
    overlap = compute_ngram_overlap(generated, target, tokenizer=DummyTokenizer(), n=3)
    assert 0.0 < overlap <= 1.0


def test_ngram_overlap_returns_zero_for_short_text():
    assert compute_ngram_overlap("tiny", "also tiny", tokenizer=DummyTokenizer(), n=5) == 0.0
